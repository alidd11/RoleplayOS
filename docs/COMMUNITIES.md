...
Private Roblox servers derive a stable community ID from `game.PrivateServerId`. The private-server owner initialises that community by supplying a Roblox group ID through the `CreatePrivateCommunity` endpoint. The server calls `GroupService:GetGroupInfoAsync` and accepts the link only when the group's owner ID matches `game.PrivateServerOwnerId`. The stored community name and emblem come from Roblox rather than client input.

Community access and administration are then derived from `GroupService:GetRolesInGroupAsync`. `Communities.GroupRoleRanks` maps Roblox ranks to Owner, Administrator, Moderator and Member permissions. API failures fail closed. Reserved servers may later be routed to an existing community session using signed TeleportData; public servers remain community-neutral.
