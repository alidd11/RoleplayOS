...
Every RoleplayOS spawn is a native `SpawnLocation` bound to a configured Team through `TeamColor`. Pads are named after their team (`Civilian`, `Police`, `Ambulance` or `Fire`). A stable `SpawnId` attribute distinguishes multiple pads for the same team and binds each one to a station and explicit permitted roles and departments. Duplicate, absent or mismatched IDs are unavailable.
